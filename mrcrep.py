...
knockoff_directory = parent_directory + '/knockpy/'
...
def fetch_competitor_S(
	...
	**kwargs
	...
	S_matrices['ci'] = knockpy.smatrix.compute_smatrix(
		Sigma=Sigma,
		method='ciknock',
	)
	if verbose:
...
def single_dataset_power_fdr(
	...
	for S_method in S_matrices:
		...
		# we overwride the knockoff_filter's attempt to prevent
		# numerical errors and sample the knockoffs ourselves.
		_sdp_degen = (check_equi_degen(dgprocess.Sigma) and S_method == 'sdp')

		# Do NOT run OLS or debiased lasso for degenerate case,
		# because this will lead to linear algebra errors.
		if _sdp_degen:
			if filter_kwargs.get('fstat', 'default') in ['dlasso', 'ols']:
		...
		# Account for equicorrelated case
		if ksampler == 'gaussian' and _sdp_degen:
		...
		kfilter.forward(
			...
			Sigma=None if infer_sigma else Sigma, # ignored by fx 
			groups=groups,
			**filter_kwargs
		)
		Z = kfilter.Z
...
def compare_S_methods(
	...
	if ground_truth and MX_flag and Sigma is not None:
		print(f"Storing SDP/MVR/MMI results")
	...
	for dgp_vals in dgp_product:
		...
		if 'x_dist' in new_dgp_kwargs:
			if new_dgp_kwargs['x_dist'] == 'gibbs':
				...
				print(f"File exists comes as : {os.path.exists(v_file)}")
				if os.path.exists(v_file):
					print(f"Loading custom Sigma for gibbs model at {v_file}")
					Sigma = np.loadtxt(f'{file_dir}/covcache/vout{p}.txt')
				else:
					print(f"No custom Sigma available for gibbs model at {v_file}: using default")
			# Save gibbs_graph
		...
		print(all_results.groupby(['S_method', 'n', 'method', 'q'])['power'].mean())
